...
export function renderJudgeReport(run: CaptureRun, verdicts: Verdict[]): string {
...
${ordered
  .map((verdict) => {
    ...
    const shot = entry?.file
      ? `<figure class="shot">${region}<img loading="lazy" src="${escapeHtml(entry.file)}" alt="${escapeHtml(verdict.route)}"></figure>`
      : '';
